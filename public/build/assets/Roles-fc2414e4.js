import { aA as LockClosedIcon, aB as LockOpenIcon, aC as PencilSquareIcon, aD as TrashIcon, e as mapActions, h as hooks, r as resolveComponent, l as createElementBlock, i as createBaseVNode, t as toDisplayString, k as createCommentVNode, j as createVNode, w as withCtx, o as openBlock, g as createBlock, m as mapGetters, ad as Fragment, ae as renderList, U as UserCircleIcon, L as axios, q as createTextVNode, p as withDirectives, am as vModelText, n as normalizeClass, s as withModifiers, z as pushScopeId, E as popScopeId, az as ChevronRightIcon, f as resolveDirective } from "./vendor-b24fd275.js";
...
const _sfc_main$3 = {
  ...
  methods: {
    ...
    triggerDeleteRole() {
      const self = this;
      const title = "<strong>Delete ".concat(this.role.name, "?</strong>");
      const html = "Are you sure you want to <strong>Delete";
      "".concat(this.role.name, "</strong>?<h6>This will delete the role.</h6>");
      const icon = "warning";
...
const _sfc_main$1 = {
  ...
  methods: {
    ...
    async updateRole() {
      ...
      await axios.patch("/api/roles/update-role/".concat(self.role.id), self.form).then(({ data }) => {
        self.$emit("roleUpdated", data.role);
        self.popToast({
          message: "Role ".concat(data.role.name, " Successfully Updated!"),
          timer: 5e3,
          icon: "success"
        });
...
const _sfc_main = {
  ...
  methods: {
    ...
    async getRoles(updatedPage = null) {
      ...
      await axios.get(
        "/api/roles-complete?page=".concat(this.pagination.current_page, "&per=").concat(this.perPage)
      ).then(({ data }) => {
    ...
    async deleteRole(value) {
      await axios.delete("/api/roles/delete/role/".concat(value.id)).then(({ data }) => {
...
//# sourceMappingURL=Roles-fc2414e4.js.map
